...
import { View, Text, Image, FlatList, TouchableOpacity, StatusBar, TextInput, ActivityIndicator } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Search, MoreVertical, CheckCheck, MessageCircleMore } from 'lucide-react-native';

import { router } from 'expo-router';
import { myConnections } from '@/api/myConections';
import { chatApiService } from '@/api/chatApi'; // Import your chat API service
import useChatStore from '@/store/chatStore';
import { useAuthStore } from '@/context/userAuthStore';
import { ChatListItemSkeletonAnimated, SkeletonBox } from '@/components/ChatListSkeleton';
    ...
    >
      ...
      <View className="flex-1 justify-center">
        ...
        <View className="flex-row justify-between items-center">
          <Text
            className={`text-sm flex-1 mr-2 ${connection.lastMessage && !connection.lastMessage.isRead && !connection.lastMessage.sentByMe
            ...
            <View className="flex-1 justify-center items-center p-8">
              <View className="bg-white rounded-xl  p-6 mx-4 w-full max-w-sm ">
               <View className="flex items-center justify-center my-10">
                <MessageCircleMore size={106} color="#6B7280" />
               </View>

                <Text className="text-gray-800 text-xl font-semibold text-center mb-3">
                  No Connections
                </Text>

                <Text className="text-gray-500 text-center mb-6 leading-5">
                  You don't have any connections yet. Start by finding hosts to chat with.
                </Text>

                <TouchableOpacity
                   onPress={()=>router.push("/(root_cleaner)/(profile)/connecttohost")}
